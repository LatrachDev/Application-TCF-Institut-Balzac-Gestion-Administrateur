...
function saveData() {
    ...
    // Initialize levels and categories if missing
    if (!user.levels) user.levels = {};
    if (!user.levels[currentLevel]) user.levels[currentLevel] = { categories: {} };
    if (!user.levels[currentLevel].categories[currentCategory]) {
        user.levels[currentLevel].categories[currentCategory] = { validation: false, attempts: 0, time: 0 };
    }

    // Update the user's progress for the current category
    user.levels[currentLevel].categories[currentCategory] = {
        validation: score === questions.length,
        attempts: user.levels[currentLevel].categories[currentCategory].attempts + 1,
        time: user.levels[currentLevel].categories[currentCategory].time + 20 // Placeholder for elapsed time
    };

    // For debugging: Log the updated user object to check changes
    console.log("Updated user data:", user);
    // Uncomment this after verifying the structure
