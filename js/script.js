...
const storedQuizQuestionsJSON = localStorage.getItem('quizQuestions');
...
if (!localStorage.getItem(`niveauxDebloques_${currentUser}`)) {
    initialiserNiveauxDebloquesPourUtilisateur(currentUser);
}


afficherNiveaux(currentUser);


function initialiserNiveauxDebloquesPourUtilisateur(userId) {
  const niveauxDebloques = {
    A1: { grammaire: true, vocabulaire: true, comprehension: true },
      A2: { grammaire: false, vocabulaire: false, comprehension: false },
      B1: { grammaire: false, vocabulaire: false, comprehension: false },
      B2: { grammaire: false, vocabulaire: false, comprehension: false },
      C1: { grammaire: false, vocabulaire: false, comprehension: false },
      C2: { grammaire: false, vocabulaire: false, comprehension: false }
  };
  localStorage.setItem(`niveauxDebloques_${userId}`, JSON.stringify(niveauxDebloques));
}
...
function afficherQuestion() {
  ...
  if (questionActuelle < nombreQuestionsTotal) {
      ...
      questionCounter.textContent = `Question ${questionActuelle + 1}/${nombreQuestionsTotal}: ${question.question}`;
      ...
      document.querySelector('.btnSuivant').disabled = useranswers[questionActuelle] === undefined;
...
function initialiserQuiz(niveauChoisi, categorieChoisie) {
  
    const niveauQuestions = quizQuestions.find(niveau => niveau.level === niveauChoisi);
    questionsRestantes = niveauQuestions.categories[categorieChoisie];
...
function terminerQuiz(userId, niveau, categorie) {
  clearInterval(timer); 

  let valide = score >= 10; 
  let niveauCourant = determinerNiveau(score); 

  sauvegarderDernierScore(score, niveauCourant);
  verifierDeblocageNiveau(userId, niveauCourant); 

  const niveauxDebloques = JSON.parse(localStorage.getItem(`niveauxDebloques_${userId}`));
  if (valide) {
      niveauxDebloques[niveau].grammaire = true; 
      niveauxDebloques[niveau].vocabulaire = true;
      niveauxDebloques[niveau].comprehension = true;
  }
  localStorage.setItem(`niveauxDebloques_${userId}`, JSON.stringify(niveauxDebloques));

  afficherResultat(valide, niveau, categorie);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelector(".btn").addEventListener("click", afficherNiveaux);
...
function verifierDeblocageNiveau(userId, niveau) {
  const niveauxDebloques = JSON.parse(localStorage.getItem(`niveauxDebloques_${userId}`));
  ...
  if (score >= 10) {
      ...
      const niveaux = Object.keys(niveauxDebloques);
      const indexNiveau = niveaux.indexOf(niveau);
      if (indexNiveau < niveaux.length - 1) {
          const niveauSuivant = niveaux[indexNiveau + 1];
          niveauxDebloques[niveauSuivant] = { grammaire: false, vocabulaire: false, comprehension: false };
      }
  }
...
function afficherNiveaux(userId) {
  document.querySelector(".quiz").style.display = "none";
  document.querySelector(".niveaux-section").style.display = "block";

  const niveauxContainer = document.querySelector('.niveaux-container');
  niveauxContainer.innerHTML = '';

  const niveauxDebloques = JSON.parse(localStorage.getItem(`niveauxDebloques_${userId}`));

  quizQuestions.forEach(niveau => {
      const niveauDiv = document.createElement('div');
      niveauDiv.classList.add('niveau');

      const niveauButton = document.createElement('button');
      niveauButton.textContent = `Niveau ${niveau.level}`;
     
      const estDebloque = niveauxDebloques[niveau.level].grammaire && niveauxDebloques[niveau.level].vocabulaire && niveauxDebloques[niveau.level].comprehension;

      niveauButton.disabled = !estDebloque;
      niveauButton.style.opacity = estDebloque ? '1' : '0.5';

      niveauButton.addEventListener('click', () => {
          afficherCategories(niveau.level); 
      });

      niveauDiv.appendChild(niveauButton);
...
document.querySelector(".btn2").addEventListener("click", () => {
  terminerQuiz(currentUser, niveau, categorie);
});


function afficherCategories(niveauChoisi) {
    const niveauxContainer = document.querySelector('.niveaux-container');
    niveauxContainer.innerHTML = '';

    const niveauQuestions = quizQuestions.find(niveau => niveau.level === niveauChoisi);

    for (const category in niveauQuestions.categories) {
        ...
        categoryButton.addEventListener('click', () => {
            initialiserQuiz(niveauChoisi, category); 
        });

        categoryDiv.appendChild(categoryButton);
        niveauxContainer.appendChild(categoryDiv);
    }
}
...
function afficherResultat(valide, niveau, categorie) {
  ...
  const message = valide ? 
      `Félicitations ! Vous avez validé le niveau ${niveau} en ${categorie}.` : 
      `Désolé, vous n'avez pas validé le niveau ${niveau} en ${categorie}. Réessayez.`;
  
  resultatContainer.innerHTML = `
      <p>${message}</p>
      <button class="btnRetour" onclick="retournerAuNiveauActuel('${niveau}', '${categorie}')">Retour au Niveau</button>
  `;
}

function retournerAuNiveauActuel(niveau, categorie) {
  document.querySelector('.resultat-container').style.display = 'none';
  afficherCategories(niveau); 
}
